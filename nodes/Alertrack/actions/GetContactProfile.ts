...
export async function profileAction(this: IExecuteFunctions) {
	...
	for (let i = 0; i < items.length; i++) {
		try {
			const id = this.getNodeParameter('id', i, 1) as number;

			const response = await this.helpers.request({
				method: 'GET',
				url: `https://api.sac.digital/v2/client/contact/info?id=${id}`,
				headers: {
